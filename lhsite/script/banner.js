...
function setFlowBanner() {
    ...
    function flowBannerAct() {
        //배너 롤링 초기화
        if (wrapWidth != '') {
            $wrap.find('.fblist').css({
                'animation': 'none'
            });
            $wrap.find('.fblist').slice(2).remove();
        }
    ...
    $wrap.on('mouseenter', function () {
        $wrap.find('.fblist').css('animation-play-state', 'paused');
    });

    // 마우스가 요소에서 빠져나갈 때 재생
    $wrap.on('mouseleave', function () {
        $wrap.find('.fblist').css('animation-play-state', 'running');
    });
}